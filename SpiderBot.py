#!/usr/bin/env python3
"""
tradebot_pro.py — Binance spot and futures trading bot
Supports automated trading on Binance spot and USDT-M futures markets with:
...
- Dynamic position sizing via ATR, adjusted for futures leverage
...
- Exponential backoff with jitter on API calls
...
- Backtest with same SL/TP rules and summary metrics for spot and futures
- WebSocket for real-time data (spot: wss://stream.binance.com, futures: wss://fstream.binance.com)
- Portfolio-level VaR risk management, adjusted for leverage
- Prometheus metrics for real-time monitoring
- Email alerts for critical events
- Fallback to BTCUSDT if no valid symbols
- Futures support with configurable leverage and isolated margin mode
Requires: python-binance, pandas, numpy, python-dotenv, websocket-client, prometheus-client, smtplib, (optional) scikit-learn
Example:
  python tradebot_pro.py --symbols BTCUSDT --interval 5m --mode paper --market-type futures --use-testnet true
  python tradebot_pro.py --auto-select both --interval 5m --mode paper --min-volume 5e7
  python tradebot_pro.py --symbols BTCUSDT --interval 5m --mode backtest --days 60 --market-type spot
Env vars:
...
class SafeBinanceFuturesClient:
    def __init__(self, api_key: str, api_secret: str, testnet: bool = False):
        base_url = "https://testnet.binancefuture.com" if testnet else "https://fapi.binance.com"
        self.client = CMFutures(api_key, api_secret, base_url=base_url)
        self.testnet = testnet
        self._symbol_info_cache: Dict[str, dict] = {}
    ...
    def call(self, fn, *args, **kwargs):
        for attempt in range(6):
            try:
                ...
                logging.warning(f"Futures API exception (attempt {attempt + 1}): {e}")
                if attempt == 5:
                    raise
                self._backoff(attempt)
            except Exception as e:
    ...
    def get_account(self):
        return self.call(self.client.account)

    def get_symbol_info(self, symbol: str):
        if symbol not in self._symbol_info_cache:
            info = self.call(self.client.exchange_info)
            self._symbol_info_cache = {s['symbol']: s for s in info['symbols']}
        return self._symbol_info_cache.get(symbol)

    def get_klines(self, symbol, interval, startTime=None, endTime=None, limit=1000):
        return self.call(self.client.klines, symbol=symbol, interval=interval,
                        startTime=startTime, endTime=endTime, limit=limit)

    def get_ticker(self):
        return self.call(self.client.ticker_24hr)

    def create_order(self, **kwargs):
        return self.call(self.client.new_order, **kwargs)

    def create_test_order(self, **kwargs):
        return self.call(self.client.new_order_test, **kwargs)

    def set_leverage(self, symbol: str, leverage: int):
        return self.call(self.client.change_leverage, symbol=symbol, leverage=leverage)

    def set_margin_type(self, symbol: str, margin_type: str = 'ISOLATED'):
        return self.call(self.client.change_margin_type, symbol=symbol, marginType=margin_type)

def create_client(api_key, api_secret, market_type: str, testnet: bool):
    if market_type == "spot":
        return SafeBinanceClient(api_key, api_secret, testnet)
    elif market_type == "futures":
        return SafeBinanceFuturesClient(api_key, api_secret, testnet)
    else:
...
def enforce_exchange_filters(client, symbol: str, qty: float, price: Optional[float]) -> Tuple[float, Optional[float], float]:
    ...
    if "NOTIONAL" in filters:
        ...
        notional = float(filters["MIN_NOTIONAL"].get("minNotional", "0.0"))
    return adj_qty, adj_price, notional

def get_min_order(client, symbol: str) -> float:
    info = client.get_symbol_info(symbol)
...
def indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tr = np.maximum(
        out["High"] - out["Low"],
        np.maximum(
            np.abs(out["High"] - out["Close"].shift()),
...
# -------- Data Helpers --------
def fetch_klines_series(client, symbol: str, interval: str, start: datetime, end: datetime) -> pd.DataFrame:
...
# -------- WebSocket Data Feed --------
class DataFeed:
    def __init__(self, symbols: List[str], interval: str, client):
        self.client = client
        ...
        for sym in self.symbols:
            try:
                ...
                self.data[sym] = df
                logging.info(f"Initialized data for {sym} ({'with history' if not df.empty else 'empty'})")
            except BinanceAPIException as e:
                logging.warning(f"Cannot initialize data for {sym}: {e}")
    ...
    def on_message(self, ws, message):
        try:
            ...
            if "k" in msg:
                k = msg["k"]
                sym = k["s"]
                if sym in self.data and k["x"]:  # Process only closed candles
                    df_new = pd.DataFrame({
                    ...
                    logging.debug(f"Received WebSocket data for {sym}")
        except Exception as e:
            logging.warning(f"WebSocket message processing error: {e}")
    ...
    def start(self):
        ws_url = "wss://stream.binance.com:9443/ws"  # Spot WebSocket URL
        ...
        self.running = True
        self.thread = threading.Thread(target=self.run_forever_with_reconnect, daemon=True)
        self.thread.start()
        # Wait for connection to establish
        time.sleep(2)  # Give 2 seconds for connection (adjust if needed)
        if self.ws.sock and self.ws.sock.connected:
            for sym in self.symbols:
                self.ws.send(json.dumps({
                    "method": "SUBSCRIBE",
                    "params": [f"{sym.lower()}@kline_{self.interval}"],
                    "id": 1
                }))
                logging.info(f"Subscribed to WebSocket for {sym}")
        else:
            logging.error("WebSocket failed to connect; check network or API settings")
            self.running = False
    ...
    def run_forever_with_reconnect(self, max_retries=10):
        ...
        while self.running and retry < max_retries:
            ...
            logging.info(f"Reconnecting WebSocket in {backoff:.2f} seconds (attempt {retry + 1})")
            time.sleep(backoff)
            retry += 1
...
# -------- Portfolio VaR --------
def portfolio_var(positions: Dict[str, dict], prices: Dict[str, float], df: Dict[str, pd.DataFrame], leverage: int = 1) -> float:
    weights = []
    returns = []
    for sym, pos in positions.items():
        if pos["qty"] > 0:
            ...
            total_notional = sum(p["qty"] * prices.get(s, p["entry_price"]) * leverage for s, p in positions.items() if p["qty"] > 0)
    ...
    portfolio_var = np.sqrt(np.dot(weights, np.dot(cov_matrix * 252, weights))) if weights else 0.0
    return portfolio_var

# -------- Sizing / PnL Helpers --------
def position_size_from_atr(balance_usdt: float, price: float, atr: float, risk_per_trade: float, leverage: int = 1) -> float:
    if atr <= 0 or price <= 0:
        return 0.0
    risk_dollars = balance_usdt * risk_per_trade * leverage
...
def get_free_usdt(client) -> float:
    try:
        ...
        if isinstance(client, SafeBinanceClient):
        ...
        logging.warning(f"Failed to get free USDT: {e}")
        return 1.0
...
# -------- Orders --------
def place_market_order(client, symbol: str, side: str, qty: float, mode: str, market_type: str) -> dict:
    try:
        ...
        if market_type == "futures":
            order_kwargs["positionSide"] = "BOTH"
        if mode == "live":
        ...
        if e.code == -2010:  # Insufficient balance
            send_alert(f"Insufficient balance for {symbol}", {"error": str(e)})
        raise

# -------- Symbol Auto-Select --------
def auto_select_symbols(client, mode: str, min_volume: float):
...
def run_live_or_paper(
    ...
    client = create_client(api_key, api_secret, market_type, testnet=(use_testnet and mode != "live"))
    ...
    if market_type == "futures":
        for sym in symbols:
            try:
                ...
                logging.info(f"Set leverage={leverage} and isolated margin for {sym}")
            except BinanceAPIException as e:
                logging.warning(f"Failed to configure futures for {sym}: {e}")
    ...
    if any(sym not in data_feed.data or data_feed.data[sym].empty for sym in symbols):
        ...
        if force_default:
            logging.warning("Attempting fallback to BTCUSDT")
            try:
                client.get_symbol_info("BTCUSDT")
                client.get_klines("BTCUSDT", interval, limit=1)
                symbols = ["BTCUSDT"]
                ...
                if market_type == "futures":
                    try:
                        client.set_leverage("BTCUSDT", leverage)
                        client.set_margin_type("BTCUSDT", 'ISOLATED')
                    except BinanceAPIException as e:
                        logging.warning(f"Failed to configure futures for BTCUSDT: {e}")
                data_feed.start()
                start_time = time.time()
                while time.time() - start_time < max_wait:
                    if all(sym in data_feed.data and not data_feed.data[sym].empty for sym in symbols):
                        break
                    logging.info("Waiting for WebSocket data for fallback symbol...")
                    time.sleep(5)
                if any(sym not in data_feed.data or data_feed.data[sym].empty for sym in symbols):
                    logging.error("Failed to get WebSocket data for BTCUSDT")
                    sys.exit(1)
            except BinanceAPIException as e:
                logging.error(f"Default symbol BTCUSDT also invalid: {e}")
                sys.exit(1)
        else:
            logging.error("No WebSocket data and force_default=False; exiting")
            sys.exit(1)

    # Paper mode test orders
    if mode == "paper":
        test_symbol = symbols[0]
        symbol_info = client.get_symbol_info(test_symbol)
        min_notional = get_min_order(client, test_symbol)
        df_ind = data_feed.data[test_symbol]
        price = float(df_ind["Close"].iloc[-1]) if not df_ind.empty else 1.0
        order_qty = max(0.001, min_notional / price)
        step_size = next((float(f["stepSize"]) for f in symbol_info.get("filters", []) if f["filterType"] == "LOT_SIZE"), None)
        ...
        try:
            ...
            resp = place_market_order(client, test_symbol, "BUY", order_qty_str, mode, market_type)
            logging.info("Test order successful")
        except BinanceAPIException as e:
    ...
    while True:
        try:
            ...
            current_prices = {
                sym: float(data_feed.data[sym].iloc[-1]["Close"])
                if sym in data_feed.data and not data_feed.data[sym].empty else 0.0
                for sym in symbols
            }

            portfolio_risk = portfolio_var(state, current_prices, data_feed.data, leverage if market_type == "futures" else 1)
            if portfolio_risk > abs(var_limit):
                send_alert("Portfolio VaR Limit Hit", {"VaR": f"{portfolio_risk:.2%}", "Timestamp": now.isoformat()})
                logging.warning("Portfolio VaR limit hit; stopping")
                break

            for sym in symbols:
                if state[sym]["disabled"]:
                    continue
                ...
                if s["position"] != 0 and s["stop"] is not None and s["take"] is not None:
                    ...
                    if hit_stop or hit_take:
                        ...
                        step_size = next((float(f["stepSize"]) for f in client.get_symbol_info(sym)["filters"] if f["filterType"] == "LOT_SIZE"), None)
                        qty_str = format_quantity(adj_qty, step_size) if step_size else str(adj_qty)
                        resp = place_market_order(client, sym, exit_side, qty_str, mode, market_type)
                        gross = trade_pnl("BUY" if s["position"] == 1 else "SELL", s["entry_price"], price, float(qty_str))
                        fees = (order_notional("BUY" if s["position"] == 1 else "SELL", s["entry_price"], float(qty_str)) + order_notional(exit_side, price, float(qty_str))) * fee_rate
                        realized = gross - fees
                ...
                if s["position"] == 0:
                    ...
                    if signal != 0 and atr > 0:
                        ...
                        min_notional = get_min_order(client, sym)
                        hard_min_dollar = 10.0
                        min_qty_by_notional = min_notional / price
                        risk_based_qty = position_size_from_atr(free_usdt, price, atr, risk_per_trade, leverage if market_type == "futures" else 1)
                        candidate_qty = max(risk_based_qty, min_qty_by_notional, hard_min_dollar / price)
                        adj_qty, _, _ = enforce_exchange_filters(client, sym, candidate_qty, price)
                        if adj_qty * price < min_notional:
                            s["small_qty_count"] += 1
                            logging.info(f"{sym}: qty too small ({adj_qty * price:.2f} < {min_notional}); skipping")
                            if s["small_qty_count"] >= 5:
                                logging.info(f"{sym}: disabled due to repeated small qty")
                                s["disabled"] = True
                            continue
                        else:
                            s["small_qty_count"] = 0
                        ...
                        resp = place_market_order(client, sym, side, qty_str, mode, market_type)
                ...
                if drawdown > 0.5:
                    ...
                    logging.warning("Max drawdown exceeded; stopping")
                    return
                peak_equity = max(peak_equity, total_equity)
                if day_realized_pnl <= daily_loss_limit * starting_day_balance:
                    send_alert("Daily Loss Limit Hit", {"Loss": f"{day_realized_pnl:.2f} USDT", "Timestamp": now.isoformat()})
...
def backtest(
    ...
    client = create_client(api_key, api_secret, market_type, testnet=True)
    ...
    if market_type == "futures":
        try:
            ...
            logging.warning(f"Failed to configure futures for backtest: {e}")
    ...
    def size_for(price, atr):
        return position_size_from_atr(balance, price, atr, risk_per_trade, leverage if market_type == "futures" else 1)
...
def parse_args():
    import argparse
    p = argparse.ArgumentParser(description="TradeBot Pro - Spot & Futures")
    ...
    p.add_argument("--daily-loss-limit", type=float, default=float(os.getenv("DAILY_LOSS_LIMIT", "-0.05")), help="Daily loss limit as fraction")
    p.add_argument("--var-limit", type=float, default=float(os.getenv("PORTFOLIO_VAR_LIMIT", "-0.05")), help="Portfolio VaR limit as fraction")
    p.add_argument("--max-hours", type=int, default=int(os.getenv("MAX_TRADE_HOURS", "24")), help="Max hours to keep position open")
    ...
    p.add_argument("--use-testnet", action="store_true", default=os.getenv("USE_TESTNET", "true").lower() in ("1", "true", "yes"))
    p.add_argument("--force-default", action="store_true", default=os.getenv("FORCE_DEFAULT", "true").lower() in ("1", "true", "yes"))
    ...
    p.add_argument("--tp-mult", type=float, default=float(os.getenv("TP_MULT", "3.0")), help="Take-profit multiple (of SL move)")
    p.add_argument("--sl-mult", type=float, default=float(os.getenv("SL_MULT", "2.0")), help="Stop-loss multiple (ATR)")
    p.add_argument("--market-type", choices=["spot", "futures"], default=os.getenv("MARKET_TYPE", "spot"), help="Market type: spot or futures")
    p.add_argument("--leverage", type=int, default=int(os.getenv("LEVERAGE", "10")), help="Futures leverage (default: 10)")
...
def main():
    ...
    if args.mode == "backtest":
        ...
        backtest(
            symbols[0], args.interval, args.days, args.risk, args.fee,
            args.tp_mult, args.sl_mult, args.use_ml, args.market_type, args.leverage
        )
    else:
        run_live_or_paper(
            symbols=args.symbols, interval=args.interval, mode=args.mode,
            risk_per_trade=args.risk, daily_loss_limit=args.daily_loss_limit,
            var_limit=args.var_limit, max_hours=args.max_hours, use_testnet=args.use_testnet,
            log_file=args.log_file if args.log_file else None, use_ml=args.use_ml,
